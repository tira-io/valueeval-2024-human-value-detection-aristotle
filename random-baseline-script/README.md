...
```bash
# build
docker build -t registry.webis.de/code-research/tira/tira-user-aristotle/random-baseline-script:1.0.0 .

# run
docker run --rm \
  -v $PWD/../toy-dataset:/dataset -v $PWD/output:/output \
  registry.webis.de/code-research/tira/tira-user-aristotle/random-baseline-script:1.0.0
